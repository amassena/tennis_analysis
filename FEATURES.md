...
| Filmstrip contact precision | `feature/visual/filmstrip-contact-precision` | `~/tennis_worktrees/filmstrip/` | scripts/swing_composite.py | active — non-uniform dense sampling shipped (5 panels at ~37ms steps around raw_contact + 6 wide). Serves and close groundstrokes now reliably show ball-at-strings in the dense zone. Remaining misses: (a) detector off by >250ms (dense zone misses entirely), (b) two-player rallies where pose tracking jumps to the wrong person. Both are upstream-ish; investigating audio-peak snap + pose disambiguation. |
