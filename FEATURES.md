...
| 3D-lifting investigation | `feature/detection/3d-lifting` | (worktree removed 2026-05-20) | scripts/audit_world_landmarks.py, scripts/render_audit_frames.py, eval/3d-lifting/ | **Closed 2026-05-20. Superseded by `feature/coaching/swing-organization`.** Phase 1 surfaced that kinetic-chain cascade hypothesis doesn't fit rally data (10% cascade rate even with correct angular-velocity impl). Audit infrastructure (`audit_world_landmarks.py` + `render_audit_frames.py`) cherry-picked to main 2026-05-20 in commit 719c1e3. |
