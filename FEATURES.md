...
| 3D-lifting investigation | `feature/detection/3d-lifting` | `~/tennis_worktrees/3d-lifting/` | scripts/audit_world_landmarks.py, scripts/render_audit_frames.py, eval/3d-lifting/ | **Closed 2026-05-20. Superseded by `feature/coaching/swing-organization`.** Phase 1 surfaced that kinetic-chain cascade hypothesis doesn't fit rally data (10% cascade rate even with correct angular-velocity impl). Audit infrastructure (`audit_world_landmarks.py`) preserved for future biomech regression tests. |
| Swing-organization coaching metric | `feature/coaching/swing-organization` | `~/tennis_worktrees/swing-organization/` | scripts/biomechanical_analysis.py, scripts/claude_coach.py | active — drops `kinetic_chain_*` fields (broken metric that fed Claude wrong values for months). Adds `wrist_contact_offset_cm` to analyze_shot: signed forward distance of dominant wrist from hip-center at contact, via world_landmarks z-axis. Positive = wrist ahead of body (good groundstroke contact). Smoke-tested on IMG_0996: FH median +17cm, BH median -6cm (matches user's known late-backhand pattern). Ready to commit + merge to main. |
| Shot-correction gallery feature | `feature/gallery/shot-corrections` | `~/tennis_worktrees/shot-corrections/` | worker/upload-worker.js, scripts/update_r2_index.py, R2 corrections/{vid}.json | active — right-click any JUMP TO SHOT chip (or 🚩 in compilation player overlay) → 4-button picker (FH/BH/SV/Not a shot) → POST /api/correction/:vid (no auth) → corrected chip shows new type + ✓ badge. Captures user-corrections as labeled training data for the active improve-shot-classification stream. |
...
| Filmstrip contact precision | `feature/visual/filmstrip-contact-precision` | `~/tennis_worktrees/filmstrip/` | scripts/swing_composite.py | shipped — non-uniform dense sampling (5 panels at ~37ms steps + 6 wide) plus audio-peak contact-frame snap (±300ms, 1.5× threshold). ~60% of IMG_1120 shots get snapped to the actual thwack, fixing ~half of previously-broken-by-detector cases. Audio cached at audio_hits/{vid}.json. Remaining misses are upstream: (a) detector off by >300ms (audio peak outside search window), (b) two-player rallies where pose tracking jumps between players. |
...
| File | Owners |
|---|---|
| `scripts/swing_composite.py` | filmstrip-contact-precision |
