...
| Sapiens pose evaluation | `feature/detection/sapiens-eval` | `~/tennis_worktrees/sapiens-eval/` | scripts/sapiens_pose.py, scripts/compare_pose_extractors.py, eval/sapiens/ | **REJECT on speed (closed snapshot).** Sapiens-2 0.4B at 0.11× MediaPipe (need ≥0.5×). Accuracy +0.0258 conf delta, dominates on motion keypoints. License: Sapiens-2 commercial-OK. Branch retained as eval record; superseded by `feature/detection/pose-experiments`. |
| Pose-extractor experiments | `feature/detection/pose-experiments` | `~/tennis_worktrees/pose-experiments/` | scripts/sapiens_pose_optimized.py, scripts/rtmpose_extract.py, eval/pose-experiments/ | **REJECT both candidates (closed snapshot).** Sapiens-2-opt: 4.0 fps (0.16× MP, speed REJECT). RTMPose: 28 fps (1.14× MP) BUT 2D-only — biomech pipeline depends on MediaPipe world_landmarks. Key finding: 2D pose isn't the bottleneck (3 models agree to 5 pixels at 1080p); the load-bearing question is MP's 2D→3D lifting. Follow-up brief proposes MotionBERT/HMR2 lifting head on top of MP's 2D. |
| 3D-lifting investigation | `feature/detection/3d-lifting` | `~/tennis_worktrees/3d-lifting/` | scripts/audit_world_landmarks.py, scripts/render_audit_frames.py, eval/3d-lifting/ | **Phase 1 complete: world_landmarks NOT the bottleneck.** Audit of 256 GT shots across 3 videos: static 3D-derived angles (knee, trunk, arm) biomechanically sound; 84% flag rate driven by kinetic-chain timing extraction bug in `biomechanical_analysis.py:174-192` (no smoothing on per-frame velocity, picks noise peaks). Phase 2 (MotionBERT/HMR2) NOT warranted. Recommended fix: smooth velocity + min-prominence peak filter in biomech_analysis. ~half-day work. |
