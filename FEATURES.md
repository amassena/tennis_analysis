...
|---|---|---|---|---|
| iPhone Shortcut ingest | (main) | (none) | worker/upload-worker.js, gpu_worker/worker.py, coordinator/api.py, coordinator/iphone_upload_poller.py | Phase 0+1 done. Worker `/api/upload/iphone` live, GPU R2-first source lookup deployed both GPUs, Hetzner `tennis-iphone-poller.service` bridges R2 markers → coordinator jobs. End-to-end via curl green (upload → marker → poller → coordinator job in <30s). Next: user builds the iPhone Shortcut + 1-week soak alongside watcher. |
