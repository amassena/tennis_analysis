...
| Pose-extractor experiments (better than MediaPipe within 0.5× speed) | `feature/detection/pose-experiments` | `~/tennis_worktrees/pose-experiments/` | scripts/sapiens_pose.py, scripts/rtmpose_extract.py (new), scripts/pose_optimizations.py (new), eval/pose-experiments/ (new) | active — testing 3 candidates: Sapiens-2-optimized (YOLO+bbox-reuse+fp16), RTMPose-l, MP tuning. Reuses sapiens-eval comparator + corpus. Goal: pose accuracy >MP on biomech indices, throughput ≥0.5× MP. |
